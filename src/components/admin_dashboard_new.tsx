...
const AdminDashboard = () => {
  ...
  const [editModes, setEditModes] = useState<{ [userId: string]: boolean }>({});
  ...
  useEffect(() => {
    ...
    fetchUserData();
    loadUserProfiles();
    // Set edit modes based on your data
    const initialEditModes: { [userId: string]: boolean } = {};
    Object.keys(users).forEach((userId) => {
      initialEditModes[userId] = false; // Initialize all users as not in edit mode
    });
  ...
  {Object.keys(users).map((userId) => (
    <tr key={userId}>
      <td id={`user-id-${userId}`}>
        {editModes[userId] ? (
          <input type="text" defaultValue={users[userId].id} />
        ) : (
          users[userId].id
        )}
      </td>
      <td id={`user-name-${userId}`}>
        {editModes[userId] ? (
          <input type="text" defaultValue={users[userId].name} />
        ) : (
          users[userId].name
        )}
      </td>
      <td id={`user-email-${userId}`}>
        {editModes[userId] ? (
          <input type="text" defaultValue={users[userId].email} />
        ) : (
          users[userId].email
        )}
      </td>
      <td id={`user-role-${userId}`}>
        {editModes[userId] ? (
          <input type="text" defaultValue={users[userId].role} />
        ) : (
          users[userId].role
        )}
      </td>
      <td id={`button-${userId}`}>
        {editModes[userId] ? (
          <button onClick={() => handleSaveClick(userId)}>Save</button>
        ) : (
          <button onClick={() => handleEditClick(userId)}>Edit</button>
        )}
      </td>
    </tr>
  ))}
  ...
  async function startChatWithUser(): Promise<void> {
    try {
      //const usersData: { [key: string]: UserData } = await fetchUserData();
    ...
    <div className="container">
      ...
      <div className="container">
        <div className="dashboard-container">
          ...
          <a href="telemetry.html" className="admin-1l-button">
            TELEMETRY
